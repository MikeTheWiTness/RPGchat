...
class TestAutoMode:
    ...
    def test_auto_pause_on_pending_checkpoint_proposal(self, setup):
        gl, provider = setup
        provider.set_responses([
            json.dumps({
                "world_setting": "世界",
            ...
            json.dumps({
                "id": "npc_1", "name": "NPC 1", "character_type": "npc",
                "personality": "沉默",
            }),
            json.dumps({
                "next_speaker": "npc_1",
        ...
        gl.propose_checkpoint("第一章")

        controller = AutoModeController(gl)
        output = controller.start()
        assert controller.is_paused is True
        assert "待确认" in output or "已暂停" in output

    def test_auto_resume_after_confirmation_resolved(self, setup):
        gl, provider = setup
        provider.set_responses([
            json.dumps({
                "world_setting": "世界",
        ...
        output = controller.resume()
        assert controller.is_paused is False

    def test_absent_mode_auto_run_pauses_at_checkpoint(self, setup):
        gl, provider = setup
        provider.set_responses([
            json.dumps({
                "world_setting": "世界",
        ...
        gl.new_game_absent("空缺自动", "世界")
        assert gl.session.mode == "player-absent"

        controller = AutoModeController(gl)
        output = controller.start()
        assert "故事开始" in output

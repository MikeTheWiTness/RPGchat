...
class TestPersistence:
    ...
    def test_character_extended_fields_preserved(self, temp_dir):
        profile = CharacterProfile(
            ...
            background="孤儿出身",
            appearance="银发",
            physique={"height": 180, "weight": 70},
            identity={"occupation": "佣兵", "social_class": "平民"},
            clothing="黑色皮甲",
            behavior={"habits": ["摸下巴"], "speech_style": "简洁"},
            intimate_features="左耳有耳洞",
        )
        session = GameSession(
            id="test-ext", name="测试", mode="player-present",
            mechanics_mode="pure-narrative",
            characters={"npc_test": profile},
            character_contexts={},
        )
        filepath = os.path.join(temp_dir, "test_ext.json")
        save(session, filepath)
        loaded = load(filepath)

        p = loaded.characters["npc_test"]
        assert p.name == "Test"
        assert p.physique == {"height": 180, "weight": 70}
        assert p.identity == {"occupation": "佣兵", "social_class": "平民"}
        assert p.clothing == "黑色皮甲"
        assert p.behavior == {"habits": ["摸下巴"], "speech_style": "简洁"}
        assert p.intimate_features == "左耳有耳洞"
        assert p.personality == "冷静"
        assert p.background == "孤儿出身"
        assert p.appearance == "银发"
